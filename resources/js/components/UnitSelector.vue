<template>
  <div class="space-y-4">
    <!-- Unit Section -->
    <div>
      <label for="unit-selector" class="block text-sm font-medium text-gray-700 mb-2">Unit:</label>
      <div class="mb-2">
...
export default {
  ...
  watch: {
    initialUnitId: {
      handler: 'initializeSelectedUnit',
      immediate: true
    }
  },
  
  methods: {
    async initializeSelectedUnit() {
      if (!this.units.length) {
        await this.loadUnits();
      }
      
      if (this.initialUnitId) {
        const unit = this.units.find(u => u.id === parseInt(this.initialUnitId));
        if (unit) {
          this.selectedUnit = unit;
          this.selectedUnitId = '';
        }
      } else {
        // Reset if initialUnitId is cleared
    ...
    setSelectedUnit(e) {
      ...
      if (unit) {
        ...
        console.warn(`Unit with ID ${this.selectedUnitId} not found`);
        this.selectedUnitId = '';
      }
    },
